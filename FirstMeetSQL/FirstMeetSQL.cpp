...
bool is_table_empty(sqlite3* db) {
	sqlite3_stmt* stmt;
	const char* sql = "SELECT COUNT(*) FROM PidorsOfRayon;";
	int count = 0;
...
int main(){
	...
	const char* create_sql = "CREATE TABLE IF NOT EXISTS PidorsOfRayon (Id INTEGER PRIMARY KEY AUTOINCREMENT, Nick TEXT NOT NULL, KubkiInBS INTEGER);";//sql-запрос для создания таблицы
	...
	const char* insert_sql = "INSERT INTO PidorsOfRayon (Nick, KubkiInBS) VALUES (?, ?);";
	...
	//if (is_table_empty(db)) {
		...
		for (const auto& user : users) {
			...
			sqlite3_bind_int(stmt, 2, user.kubki);//заменяем второй "?" на число кубков в бравле 
			sqlite3_step(stmt); //переходим на следующую строчку в таблице
			sqlite3_finalize(stmt); //нужная херня
		}
	//}

	cout << "------ Ебланы года, вот же они -------\n";
	const char* sel_all = "SELECT * FROM PidorsOfRayon;"; //выбрать всё из нашей таблицы
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) { // sqlite3_step() возвращает SQLITE_ROW, пока есть данные для чтения
		...
		const unsigned char* nick = sqlite3_column_text(stmt, 1);//unsigned - здесь, чтобы не наебнулась кодировка, а так можно юзать для только положительных интов (по типу возраста, цены и т.п) | Колонка 1 - Ник (text)
